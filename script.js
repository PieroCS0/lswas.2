...
(function () {
  ...
  /* ---------------------------------------------------------
     Sistema de órbitas: girasoles girando alrededor del sol
     --------------------------------------------------------- */
  ...
  var geom = { cx: 0, cy: 0, usableStart: 60, usableRange: 100 };
  ...
  function addOrbiter(delayMs, ringFraction) {
  ...
  function positionOrbiter(o, elapsedSec) {
    ...
    var angleDeg = o.baseAngle + o.speed * elapsedSec + globalSpin;
    var rad = angleDeg * Math.PI / 180;
    var x = geom.cx + radius * Math.cos(rad);
    var y = geom.cy + radius * Math.sin(rad);
    o.el.style.transform = 'translate(' + (x - o.size / 2).toFixed(1) + 'px, ' + (y - o.size / 2).toFixed(1) + 'px)';
  }

  var orbitStartTime = null;
  var globalSpin = 0;          // grados acumulados por el arrastre del usuario
  var globalSpinVelocity = 0;  // grados/seg, para la inercia al soltar

  function orbitLoop(ts) {
    if (orbitStartTime === null) orbitStartTime = ts;
    var elapsedSec = (ts - orbitStartTime) / 1000;

    if (!dragState.active && Math.abs(globalSpinVelocity) > 0.5) {
      var dt = orbitLoop._lastTs ? (ts - orbitLoop._lastTs) / 1000 : 0.016;
      globalSpin += globalSpinVelocity * dt;
      globalSpinVelocity *= Math.pow(0.02, dt); // fricción: decae rápido pero suave
    } else if (!dragState.active) {
      globalSpinVelocity = 0;
    }
    orbitLoop._lastTs = ts;
  ...
  /* ---------------------------------------------------------
     Arrastre táctil: girar todo el universo con el dedo
     --------------------------------------------------------- */
  var dragState = {
    active: false,
    pointerId: null,
  ...
  function onScenePointerMove(e) {
    ...
    if (dragState.moved) {
      ...
      globalSpin += dx * degPerPx;
      globalSpinVelocity = (dx / dt) * degPerPx * 1000;
  ...
  function onScenePointerUp(e) {
    ...
    if (dragState.moved) {
      globalSpinVelocity = clamp(globalSpinVelocity, -900, 900);
      suppressNextClick();
    }
  }
